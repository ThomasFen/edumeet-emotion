          ...
          try {
            ...
            if (analysisLatency > peer.emotion.msPerFrame) {
              ...
              if (peer.emotion.droppedResults === 5) {
                logger.debug(
                  'Reached upper limit of allowed consecutive result drops because of high latency. Lowering fps ... [latency:"%o" ]',
                  analysisLatency
                );
                ...
                logger.debug(
                  'Dropping result because of high latency. [latency:"%o" ], [#droppedResults:"%o" ]',
                  analysisLatency,
            ...
            if (reply.output.length > 0) {
              logger.debug(
                'BentoML reply containing results [#Emotions:"%o"]',
                reply.output.length
              );
              // A relativeBox implies that face detection took place before bentoML analysis
              const result = relativeBox
                ? { ...reply.emotions, boxes: [relativeBox] }
                : reply.emotions;
              ...
              socketio
                ...
                .volatile.emit("emotion", JSON.stringify(result));
            }
          } catch (error) {
