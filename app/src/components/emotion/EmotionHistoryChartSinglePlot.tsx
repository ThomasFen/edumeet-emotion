...
function EmotionHistoryChartSinglePlot({ data, width, height }) {
        ...
        <div style={{ position: 'relative' }}>
            ...
            <rect x={0} y={0} width={width} height={height} fill={'#718096'} rx={14} />
                <Group left={margin.left} top={margin.top}>
                    ...
                    {series.map((emotionSeries, i) => (
                        <LinePath
                            key={i}
                            stroke={colors[i]}
                            strokeWidth={3}
                            data={sData}
                            x={(d) => timeScale(getX(d)) ?? 0}
                            y={(d) => rdScale(getY(d)) ?? 0}
...
export default EmotionHistoryChartSinglePlot